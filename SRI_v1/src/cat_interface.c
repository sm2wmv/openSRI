  ...
  if (cat_interface.radio_model == RADIO_MODEL_YAESU_FT950) {
    if (length == 26) {
      if (strncmp((char *)buffer,(char *)"IF",2) == 0) {
        ...
        #ifdef CAT_FREQ_DEBUG
              PRINTF("CAT DATA: FREQ VFO A: %i\n",freq_vfoA);
              PRINTF("CAT DATA: MODE VFO A: %c\n",buffer[20]);
        #endif
        ...
        #ifdef CAT_FREQ_DEBUG
              PRINTF("CAT DATA: FREQ VFO B: %i\n",freq_vfoB);
              PRINTF("CAT DATA: MODE VFO B: %c\n",buffer[20]);
        #endif
...
uint8_t cat_interface_uart_poll_computer_rx(uint8_t *data) {
  if (!(LPC_UART0->LSR & UART_LSR_RDR)) {
    ...
    data[0] = UART_ReceiveByte(LPC_UART0);
  }
