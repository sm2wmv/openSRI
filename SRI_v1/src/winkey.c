...
uint8_t winkey_init_sequence(uint8_t state) {
  PRINTF("WK: INIT SEQUENCE: 0x%02X\n",state);
  ...
  if (state == WINKEY_STATE_ECHO_REQ) {
    ...
    winkey_send_char(settings_get_ptt_radio_pre_delay());
    winkey_send_char(0);
  ...
  PRINTF("WK: RETURN STATE FAILED\n");
  return(WINKEY_STATE_FAILED);
}

uint8_t winkey_init_failed(uint8_t state) {
  PRINTF("WK: INIT FAILED 0x%02X\n",state);

  if (state == WINKEY_STATE_ECHO_REQ) {
    PRINTF("WK: INIT FAILED, WRONG ECHO\n");
  }
  else if (state == WINKEY_STATE_REV_REPLY) {
    PRINTF("WK: INIT FAILED, WRONG REVISION\n");
  }
  ...
  if (winkey_state == WINKEY_STATE_ECHO_REQ) {
    ...
    if (winkey_rx_data != -1) {
      if (winkey_rx_data == 0x55)
        ...
        PRINTF("WK: INIT FAILED #1, WRONG ECHO 0x%02X\n",winkey_rx_data);
    ...
    if (computer_rx_len != 0) {
      for (uint8_t i=0; i<computer_rx_len;i++) {
        PRINTF("WK: TX -> 0x%02X\n",winkey_computer_rx_buffer[i]);
      }
    ...
    if (winkey_rx_data != -1) {
      if ((winkey_rx_data & 0xc0) == 0xc0) {
        if (winkey_rx_data & (1<<0))
          PRINTF("WK: STATUS -> XOFF\n");
        if (winkey_rx_data & (1<<1))
          PRINTF("WK: STATUS -> BREAKIN\n");
        if (winkey_rx_data & (1<<2))
          PRINTF("WK: STATUS -> BUSY TX CW\n");
        if (winkey_rx_data & (1<<3))
          PRINTF("WK: STATUS -> KEYDOWN\n");
        if (winkey_rx_data == 0xC0)
          PRINTF("WK: STATUS -> NO TX ACTIVE\n");
      }
...
uint32_t winkey_receive_from_computer(uint8_t *buffer, uint16_t length) {
  return(UART_Receive(LPC_UART2, buffer, length, NONE_BLOCKING));
}
...
void winkey_setting_changed(uint8_t cmd) {
  PRINTF("WK: SETTING CHANGED -> 0x%02X\n",cmd);
